...
import { Plus, Calendar, Building2, ChevronLeft, ChevronRight } from 'lucide-react'
...
export default function Cards() {
  ...
  const { data: cardsData, isLoading } = useQuery<PaginatedResponse<EventCard>>({
    queryKey: ['cards', selectedCategory, selectedScope, selectedDepartmentId, currentPage],
    queryFn: async () => {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Управляйте мероприятиями и отслеживайте их выполнение
          </p>
        </div>
        <Button asChild>
          <Link to="/cards/new" className="flex items-center">
      ...
      {cards.length > 0 ? (
        ...
        <Card>
          <CardContent className="py-12 text-center">
            <p className="text-muted-foreground">Пока нет созданных мероприятий</p>
          </CardContent>
        </Card>
      )}
    </div>
